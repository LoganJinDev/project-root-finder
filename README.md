...
- 🔍 **Find root by name** – specify your project's folder name (e.g. `"project-root-finder"`)
...
```bash
pip install git+https://github.com/LoganJinDev/project-root-finder.git
# or with a specific tag/branch
pip install git+https://github.com/LoganJinDev/project-root-finder.git@v0.1.0
```
...
# Automatically find the root folder "project-root-finder"
BASE_DIR = find_project_root_by_name(
    pathlib.Path(__file__).resolve(),
    project_name="project-root-finder"
)
...
```bash
export PROJECT_NAME=project-root-finder
```

```python
BASE_DIR = find_project_root_by_name(pathlib.Path(__file__).resolve())
# picks up 'project-root-finder' from $PROJECT_NAME
```
...
Feel free to open issues or pull requests on [GitHub](https://github.com/LoganJinDev/project-root-finder).
